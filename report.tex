...
\closein\infile
\subsection{}
As seen in~\vref{fig:f}, the cluster number density has a large linear part when \(p\to p_\mathrm{c}\). Both axes in this figure are logarithmic, so the linearity implies a power-law behaviour for \(n(s,p_\mathrm{c})\), i.e. \(n(s,p_\mathrm{c})\propto s^{-\tau}\). \Vref{fig:g} shows \(n(s,p_\mathrm{c})\) with \(L^i\), \(i\in\qty{4,\dots,9}\), as well as the result of a linear fit for the largest system size. The linear part is found by staring at the graph, and the resulting exponent is \(\tau=\num{\mytau}\).
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \begin{axis}[
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Cluster number density as a function of size for probabilities just above \(p_\mathrm{c}\) with \(L=512\) and \(\num{500}\) Monte Carlo cycles. The dots mark the numerical estimates of \(s_\xi\).}%
    \label{fig:h}
\end{figure}
