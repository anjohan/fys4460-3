...
%
\subsection{}
In this problem we are interested in calculating the inverse of \(\Pi(p,L)\), i.e.\ for a given number \(x\) determining \(p_{\Pi=x}\) such that \(\Pi(p_{\Pi=x},L) = x\). I have chosen to implement this with the bisection method, as it is very simple but also quickly converging --- the error is halved with every calculation of \(\Pi\).
\lstinputlisting[linerange={invPIstart-invPIend}]{lib/clusterlabelling.f90}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Inverse of the percolation probability \(x = \Pi(p,L)\) as a function of \(L\) for two different values of \(x\).}%
    \label{fig:l}
\end{figure}
