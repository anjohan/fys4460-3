...
%
\subsection{}
The probability of having a spanning cluster, \(\Pi(p,L)\), and the density of the spanning cluster, \(P(p,L)\), were estimated with the Fortran machinery described in the previous exercise. A variety of system sizes \(L\) and numbers of samples were used for probabilities in the range from zero to one. See~\vref{fig:PI} and~\vref{fig:PL}.
\begin{figure}[bth]
    \centering
...
%
\subsection{}
The cluster number density algorithm from the previous exercise was used.
\tikzexternaldisable
\begin{figure}[tbh]
    \centering
    \begin{tikzpicture}
        \begin{axis}[
            xmode=log, ymode=log,
            ymin=0.000000001,
            legend style={draw=none,at={(0.9,0.9)}},
            legend cell align=left,
            thick,
            %axis lines=middle,
            xlabel style={anchor=west},
            ...
            \begin{luacode*}
                ...
                for line in io.lines("tmp/f_p.dat") do
                    i = i + 1
                    print(line)
                    txt = string.format("\\addplot+[mark=none] table {tmp/f_p_%d.dat}; \\addlegendentryexpanded{\\(p = \\num{%s}\\)};", i, line)
                    tex.sprint(txt)
                end
            \end{luacode*}
        \end{axis}
