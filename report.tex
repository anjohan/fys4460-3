...
Fortran does, unfortunately, not have an \lstinline{np.polyfit} function, so the linear fit was done manually by calling a least squares subroutine from LAPACK.
...
\begin{figure}[tbh]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Spanning cluster density as a function of the probability \(p\) with \(L=\num{256}\) and \(\num{1000}\) cycles for probabilities in the region just above the critical probability. The result is compared with \(c\qty(p-p_c)^\beta\), where \(\beta\) is found by doing a linear fit on a logarithmic scale. A fairly good fit is found, at least for probabilities further away from the critical probability.}%
    \label{fig:cP}
\end{figure}
