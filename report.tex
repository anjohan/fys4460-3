...
\begin{equation}
    n\left(\left[s,s+\Delta s\right),p\right) = \displaystyle\sum_{\text{cycles}}\frac{\text{number of clusters with size between } s \text{ and } s+\Delta s}{\Delta s \cdot \text{number of cycles}}.
\end{equation}
...
\begin{figure}[tbh]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            xmode=log, ymode=log,
            ymin=0.000000001,
            legend style={draw=none,at={(0,0)},anchor=south west},
            legend cell align=left,
            thick,
            %axis lines=middle,
            xlabel style={anchor=west},
